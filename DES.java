class DES {
	...
	private static char[][] generateSubkeys(char[] key) {
		...
		for(int i=0; i<56; i++) 
			temp[i] = key[Constants.PC_1[i]-1];

		// applying left shifts and permuting again (56 bit -> 48 bit)
		for(int i=0; i<16; i++) {
			if(Constants.LEFT_SHIFT[i] == 1) {
				char swapVar = temp[0];
				for(int j=1; j<28; j++)
					temp[j-1] = temp[j];
				temp[27] = swapVar;
			...
			// permute using PC_2
			for(int j=0; j<48; j++) 
				subkeys[i][j] = temp[Constants.PC_2[j]-1];
		}
		return subkeys;
	}
	...
	private static char[] func(char[] R, char[] subkey) {
		...
		for(int i=0; i<48; i++) 
			temp[i] = R[Constants.E_BIT[i]-1];

		try{
			for(int i=0; i<48; i++)
				temp[i] = Arithmetic.XOR(temp[i], subkey[i]);
		}
		...
		for(int i=0; i<8; i++) {
			...
			String binaryTemp = Integer.toBinaryString(Constants.S[i][row][col]);
		...
		for(int i=0; i<32; i++)
			charOutput[i] = temp[Constants.P[i]-1];
	...
	public static String encrypt(String plainText, String key) {
		return crypt(plainText, key, Constants.ENCRYPT);
	}

	public static String decrypt(String cipherText, String key) {
		return crypt(cipherText, key, Constants.DECRYPT);
	}
	...
	private static String crypt(String inputText, String key, String type) {
		...
		String binary;
		binary = Conversions.hexToBinary(inputText);

		int blocksNo = (binary.length()%Constants.BLOCK_SIZE)==0 ? (binary.length()/Constants.BLOCK_SIZE):(binary.length()/Constants.BLOCK_SIZE+1);
		...
		String keyBitsString = Conversions.asciiToBinary(key);
		for(int i=0; i<64; i++)
		...
		for(int i=0; i<blocksNo; i++) {
			...
			for(int j=0; j<64; j++) {
				message[j] = blocks[i][Constants.IP[j]-1];
				if(j<32)
					_L[j] = message[j];
				else
					_R[j-32] = message[j];
			...
			for(int j=0; j<16; j++) {
				...
				try {
					for(int k=0; k<32; k++)
						R[k] = Arithmetic.XOR(_L[k], funcOutput[k]);
				}
				catch(BinaryRepresentationException e) {
					System.out.println(e);
				}
			...
			// final permutation
			for(int j=0; j<64; j++)
				message[j] = temp[Constants.IPFinal[j]-1];
		...
		for(int i=0; i<outputBits.length(); i=i+8) {
			...
			String hex = Integer.toHexString(Integer.parseInt(outputBits.substring(i, i+8), 2));
			if(hex.length()<2)
				hex = "0"+hex;
			outputText.append(hex);
		}
